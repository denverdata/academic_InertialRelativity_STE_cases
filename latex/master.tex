...
Several distinct, common forms of time dilation utilize the same formula, and that is that the time dilation is equal to $\sqrt{1-k}$, the square root of one minus the scale factor. In general relativity, the scale factor is calculated by $r_{\mathrm{s}}/R$. In special relativity, it's calculated as $v^2/c^2$, and in general relativity, you can also substitute that: $r_{\mathrm{s}}/R$ could be replaced with $D/D_{\mathrm{crit}}$. Those are all equivalent.
...
Outline. General relativity tells us that that the time dilation is fundamentally a comparison of the pace of time between two different systems. In the case of the well-known gravitational time dilation formula, where dt over d tau is equal to the square root of 1 minus 1. R sub s over r. But if you look at the formula deeper it comes out that you're comparing this system of identical mass over the radius that is equivalent to its short field radius. 
...
$d\tau/dt = \sqrt{1-k}$
where $k=r_{\mathrm{s}}/R=2GM/(Rc^2)$
...
$D$ & DeGerlia Inertial Density $M/R_{\mathrm{axis}}$ \\
$D_{\mathrm{crit}}$ & DeGerlia Threshold $c^2/2G=6.73295\times10^{26}$\,kg/m \\
DTD & DeGerlia Time Dilation DTD=$\sqrt{k}$  \\
$G$ & Gravitational Constant $(6.67430 \pm 0.00015)\times10^{-11}\ \mathrm{m^3kg^{-1}s^{-2}}$ \\
GTD & Gravitational Time Dilation $d\tau/dt = \sqrt{1-k}$ \\
$k$ & Linear Scale Factor $v^2/c^2=r_{\mathrm{s}}/R=D/D_{\mathrm{crit}}$ \\
$k_{\mathrm{D}}$ & Linear Scale Factor (DeGerlia inertial density) $D/D_{\mathrm{crit}}$ \\
$k_{\mathrm{s}}$ & Linear Scale Factor (Schwarzschild) $r_{\mathrm{s}}/R$ \\
$k_{\mathrm{v}}$ & Linear Scale Factor (velocity) $v^2/c^2$ \\
$P$ & Inertial Density $I/V$ $=D\cdot 3/10\pi$ \\
$r_{\mathrm{s}}$ & Schwarzschild Radius $2GM/c^2$  \\
STE & Spacetime Equivalence $k_{\mathrm{D}} = m_1 r_2 / m_2 r_1$ \\
\bottomrule
\end{tabular}
...
\item $k = D/D_{\mathrm{crit}} = r_{\mathrm{s}}/R$ (Equation~\ref{eq:ks}) --- via Schwarzschild radius
\end{itemize}
...
\textbf{\textit{Inertial density $P$}}\cite{degerlia202505} as shown in \eqref{eq:longinertialdensity} is a fundamental system property characterizing the distribution of inertia across any system, whether in the linear or rotational context, in any direction, or about any axis. If we were to redefine classical density as I/v (moment of inertia/volume), because inertia is mass for one-dimensional motion, this would remain consistent with the classical definition of density, where $\rho = m/v$.                     
...
\subsection{DeGerlia Inertial Density $D$}
...
Inertial density\cite{degerlia202505}, $P$, is the moment of inertia of a system divided by its volume:
\begin{equation}
P = I/V
\end{equation}
...
P = \frac{M}{R} \times \frac{3}{10\pi}
\end{equation}
Just as ordinary density ($\rho = M/V$) is linear inertia (mass) the distributed over volume, DeGerlia inertial density is rotational inertia distributed over volume. As stated in the prior work: \textit{mass and density are to linear motion as moment of inertia and inertial density are to rotational motion.}
...
Setting $R = r_{\mathrm{s}}$ (the Schwarzschild condition) and solving yields the \textbf{DeGerlia Threshold}: the universal constant of DeGerlia inertial density at which the escape velocity equals the speed of light:
\begin{equation}
D_{\mathrm{crit}} = \frac{c^2}{2G} = (6.73295 \pm 0.00015) \times 10^{26} \text{ kg/m}
\end{equation}
Note this value is exact; its precision is limited by the gravitational constant\cite{codata2018}, $G = (6.67430 \pm 0.00015) \times 10^{-11}$ m$^3$\,kg$^{-1}$\,s$^{-2}$.
...
\[
k_{\mathrm{D}} = \frac{M_1 R_2}{M_2 R_1\, D_{\mathrm{crit}}} = \frac{R_2}{R_1\, D_{\mathrm{crit}}}
\]
...
\[
k_{\mathrm{D}} = \frac{M_1 R_2}{M_2 R_1\, D_{\mathrm{crit}}} = \frac{M_1}{M_2\, D_{\mathrm{crit}}}
\]
...
When the two systems share the same density, $M \propto R^3$, so $M_1/M_2 = (R_1/R_2)^3$. Substituting into $k_{\mathrm{D}}$:
\[
k_{\mathrm{D}} = \frac{M_1 R_2}{M_2 R_1\, D_{\mathrm{crit}}} = \left(\frac{R_1}{R_2}\right)^3 \cdot \frac{R_2}{R_1\, D_{\mathrm{crit}}} = \frac{1}{D_{\mathrm{crit}}}\left(\frac{R_1}{R_2}\right)^2
\]
The DeGerlia inertial density ratio reduces to the square of the radius ratio; the linear scale factor follows as $\sqrt{k_{\mathrm{D}}} = R_1/R_2$, recovering the isometric equivalence.
...
When the two systems share the same moment of inertia, $M_1 R_1^2 = M_2 R_2^2$, so $M_1/M_2 = (R_2/R_1)^2$. Substituting into $k_{\mathrm{D}}$:
\[
k_{\mathrm{D}} = \frac{M_1 R_2}{M_2 R_1\, D_{\mathrm{crit}}} = \left(\frac{R_2}{R_1}\right)^2 \cdot \frac{R_2}{R_1\, D_{\mathrm{crit}}} = \frac{1}{D_{\mathrm{crit}}}\left(\frac{R_2}{R_1}\right)^3
\]
...
\[
k_{\mathrm{D}} = \frac{M_1 R_2}{M_2 R_1\, D_{\mathrm{crit}}}
\]
