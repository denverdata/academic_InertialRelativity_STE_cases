...
\includegraphics{head_foot/dates} & \\
 & \vspace{0.3cm} \\
 & \noindent\normalsize{ABSTRACT: In this paper, we show that the time dilation between two systems is defined solely by their relative spatial distribution of mass. Specifically, the time dilation between these systems is governed by the relative DeGerlia Inertial Density $D$, defined as mass divided by mean radius about the compared axis. We exemplify this comparative nature with one of the most fundamental formulas in general relativity, the gravitational time-dilation equation, which determines a system's time dilation relative to flat space-time: $d\tau/dt = \sqrt{1 - r_{s}/R}$. We use this reference value to validate a calculation we perform using relative $D$ alone, confirming valid predictions for a set of representative examples across the classical and relativistic regimes. In this demonstration, we introduce the DeGerlia Time Dilation Framework, a simple, mathematically equivalent framework fully defined by general relativity, offering a uniqely insightful perspective on the relative nature of time dilation.} \\
 & \vspace{0.15cm} \\
