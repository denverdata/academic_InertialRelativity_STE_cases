...
\begin{equation} \label{eq:gtd}\label{eq:td_k}
...
$k$ is a single linear scale factor: a ratio of two lengths about an axis. The same ratio that scales length scales time, which is why one quantity governs both. It is measured in three equivalent ways,
\begin{equation} \label{eq:k}
k = \frac{r_{s}}{R} = \frac{D}{D_{crit}} = \frac{v^2}{c^2},
\end{equation}
the same number arrived at from the Schwarzschild radius, from the DeGerlia inertial density $D$ against its threshold $D_{crit} = c^2/2G$, or from velocity. A scale factor is inherently a comparison: $k$ always relates one system to another, whether the second system is an explicit body or a reference condition such as $D_{crit}$.

Time dilation is recovered from $k$ in the two complementary forms above, $\mathrm{GTD} = \sqrt{1-k}$ (Equation~\ref{eq:gtd}) and $\mathrm{DTD} = \sqrt{k}$ (Equation~\ref{eq:dtd}). The DTD form is a plain power of $k$, so it carries between systems freely. The GTD form is not: because of the $1-k$, each system's $k$ is formed on its own, and only then combined. Accordingly, a comparison of two systems in GTD is the ratio of their individually computed values,
\begin{equation} \label{eq:gtd_ratio}
\frac{\mathrm{GTD}_1}{\mathrm{GTD}_2} = \frac{\sqrt{1 - k_1}}{\sqrt{1 - k_2}},
\end{equation}
where $k_1$ and $k_2$ may each be formed by either route of Equation~\ref{eq:k}. The worked examples that follow compute this ratio from both the DeGerlia inertial density $D$ and the Schwarzschild radius, and confirm they agree.
