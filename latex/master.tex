...
\includegraphics{head_foot/dates} & \\
 & \vspace{0.3cm} \\
 & \noindent\normalsize{ABSTRACT: In this paper, we show that the time dilation between two systems is defined solely by their relative spatial distribution of mass. Specifically stated, the time dilation between two systems is governed by the relative DeGerlia Inertial Density $D$, defined as mass divided by mean radius about the compared axis. We first exemplify this comparative nature with one of the most fundamental equivalences in general relativity, the gravitational time-dilation equation, which determines a system's time dilation relative to flat space-time: $d\tau/dt = \sqrt{1 - r_{s}/R}$. We use this reference value to validate the calculation we perform using relative $D$ alone. We then validate correct calculations for a diverse set of two-system examples, including transforemd systems as well as discrete system comparison, across the classical and relativistic regimes. In this demonstration, we introduce the DeGerlia Time Dilation Framework, a simple, mathematically equivalent framework fully defined by general relativity, offering a uniqely insightful perspective on the relative nature of time dilation.} \\
 & \vspace{0.15cm} \\
