...
\item $DTD$ is mathematically very easy to use and visualize, requiring only the ratio of mass to radius in comparison to a static constant to perform gravitational time dilation calculations. 
\item $DTD$ allows for direct calculation of time dilation between two systems, without the need to reference a third system (the Schwarzschild condition)
\item $DTD$ returns finite output over the entire range of finite input.
\item $DTD$ represents flat spacetime as an ideal state that can only be approached.
\item $DTD$ represents the Schwarzschild condition as the point where the gravitational escape velocity exceeds the speed of light, and an event horizon forms. While we have observed black holes, the asymptotically approached ideal state of zero time dilation is not attainable.
\item $DTD$ reflects the singularity state as an asymptotic approach to $r=0$.
\end{itemize}
...
Inertia and moment of inertia has some interesting properties. 
1. it's dimensional. And 
2. it's also multimodal. Dimensional means it exists. And it has a component within each of the three spatial dimensions. Multimodal means that it can be the rotational or linear. And as stated above, linear reflects just the mass, whereas rotational reflects the mass times radius squared about an axis. 
3. If one measures the moment of inertia about a particular axis and then separates the system into different parts, and measures their individual moments of inertia about that same axis, the total is always the same.
4. By definition, inertia dictates the delta in motion resulting from the application of a force or torque to the system. meaning inertia is the unique bridge between force and motion. Lower inertia means more motion can be achieved with the same amount of force.
...
\begin{mdframed}
\noindent\textbf{Rapid evaluation of the black hole condition.} For a single system, divide its mass by its radius and compare the result with $D_{crit} = \num{6.73295e26}\,\unit{\kilogram\per\meter}$. If their ratio equals $1$, the system is at the black hole condition; if it is less than $1$, no collapse has occurred.
\end{mdframed}

\subsection{DeGerlia Time Dilation}
Gravitational time dilation is the surviving proper-time rate:
\begin{equation} \label{eq:gtd}
\frac{d\tau}{dt} = \mathrm{GTD} = \sqrt{1-k}
\end{equation}
DeGerlia Time Dilation is its Pythagorean complement, bound to GTD by
\begin{equation} \label{eq:dtd}
\mathrm{DTD} = \sqrt{k}, \qquad \mathrm{GTD}^2 + \mathrm{DTD}^2 = 1.
\end{equation}

\subsection{The Linear Scale Factor $k$}\label{sec:k}
The following identity relates the DeGerlia time dilation between two uniform spherical systems to their masses and radii.
\begin{equation} \label{eq:kd}
k  = \frac{D_1}{D_2\,D_{crit}} = \frac{M_1 R_2}{M_2 R_1\,D_{crit}}
\end{equation}
where $R_1, R_2$ are the radii, $M_1, M_2$ are the masses, $D_i = M_i/R_i$ is the DeGerlia inertial density of system $i$, and $D_{crit} = c^2/2G$. The two forms are algebraically identical: $D_1/(D_2 D_{crit})$ expresses the comparison through the inertial density ratio, and $M_1 R_2/(M_2 R_1 D_{crit})$ is its fully expanded form. This combined ratio is the scale factor for the DeGerlia time dilation $DTD = \sqrt{k}$, where the ratio of the two systems enters directly.

The gravitational time dilation follows a different procedure. Because $GTD = \sqrt{1-k}$ carries the two systems through a nonlinear step, each system's scale factor is formed on its own,
\begin{equation} \label{eq:ks}
k_i = \frac{D_i}{D_{crit}} = \frac{r_{s,i}}{R_i},
\end{equation}
where $D_i = M_i/R_i$ is the system's DeGerlia inertial density and $r_{s,i} = 2GM_i/c^2$ is its Schwarzschild radius. Each $k_i$ yields its own gravitational time dilation $GTD_i = \sqrt{1 - k_i}$, and the comparison between the two systems is the ratio of these individual values,
\begin{equation} \label{eq:gtd_ratio}
\frac{GTD_1}{GTD_2} = \frac{\sqrt{1 - k_1}}{\sqrt{1 - k_2}}.
\end{equation}
Equivalently, since $k_i = DTD_i^2$, the same ratio is $\sqrt{1 - DTD_1^2}\,/\,\sqrt{1 - DTD_2^2}$, the form used in the worked examples that follow.

Each system's gravitational time dilation is always:
\begin{equation} \label{eq:td_k}
\frac{d\tau}{dt} = \sqrt{1 - k}
\end{equation}
where $d\tau/dt$ is the surviving proper-time rate and $k$ is that system's scale factor.
