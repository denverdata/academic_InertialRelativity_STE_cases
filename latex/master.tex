...
Compactness is the dimensionless ratio of a system's mass to its radius, scaled by $G/c^2$. The most common forms are $C = GM/(Rc^2)$ and $r_{\mathrm{s}}/R = 2GM/(Rc^2) = 2C$, where $r_{\mathrm{s}} = 2GM/c^2$ is the Schwarzschild radius. Both measure how close a system sits to its Schwarzschild threshold: $r_{\mathrm{s}}/R = 1$ marks the gravitational-collapse condition. However, because compactness is normalized, it it treats our scale as privileged, and thus begins to offer a less and less accurate view the further one moves from our own scale. 
...
$d\tau/dt = \sqrt{1-k}$
where $k=r_{\mathrm{s}}/R$=<Replace withfull version>
...
The DeGerlia Time Dilation Framework is an analogous framework that is directly derived from general relativity and has a direct relationship to gravitational time dilation. DeGerlia Time Dilation DTD shares a Pythagorean relationship with GTD.

\subsection{Definitions used within the Deerlia Time Dilation Framework  }
...
$k$ & $v^2/c^2=r_{\mathrm{s}}/R=D/D_{\mathrm{crit}}$ \\
$D$ & $M/R_{\mathrm{axis}}$ (DeGerlia inertial density) \\
...
\subsection{DeGerlia Inertial Density} 
...
\newpage
\subsection{Case 1: ($M_1 = M_2$, $R_1 = \tfrac{4}{3}r_{\mathrm{s}}$)}
...
\newpage
\subsection{Case 2: ($M_1 = M_2$, $R_1 \approx r_{\mathrm{s}}$)}
