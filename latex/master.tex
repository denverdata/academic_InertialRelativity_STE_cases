...
Several distinct, common forms of time dilation utilize the same formula, and that is that the time dilation is equal to $\sqrt{1-k}$, the square root of one minus the scale factor. In general relativity, the scale factor is calculated by $r_{s}/R$. In special relativity, it's calculated as $v^2/c^2$, and in general relativity, you can also substitute that: $r_{s}/R$ could be replaced with $D/D_{crit}$. Those are all equivalent.
...
d\tau/dt = \sqrt{1-k}
\end{equation}
where $k = r_{s}/R = 2GM/(Rc^2)$, $G$ is the gravitational constant, $M$ the system's mass, $R$ its radius, $r_{s} = 2GM/c^2$ its Schwarzschild radius, and $c$ the speed of light. The speed of light is exact by definition, $c = 299{,}792{,}458$~m/s, while the gravitational constant carries a measured uncertainty\cite{codata2018}, $G = (6.67430 \pm 0.00015) \times 10^{-11}$~m$^3$\,kg$^{-1}$\,s$^{-2}$.
...
$D_{crit}$ & DeGerlia Threshold $c^2/2G=6.73295\times10^{26}$\,kg/m \\
DTD & DeGerlia Time Dilation DTD=$\sqrt{k}$  \\
$G$ & Gravitational Constant $(6.67430 \pm 0.00015)\times10^{-11}$\ m$^3$\,kg$^{-1}$\,s$^{-2}$ \\
...
STE & Spacetime Equivalence $k_{D} = m_1 r_2 / m_2 r_1$ \\
\bottomrule
\end{tabular}
...
Setting $R = r_{s}$ (the Schwarzschild condition) and solving yields the \textbf{DeGerlia Threshold}: the universal constant of DeGerlia inertial density at which the escape velocity equals the speed of light:
\begin{equation}
