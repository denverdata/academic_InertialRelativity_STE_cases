...
\newpage
\section{Conclusions}
Observaional, mathematical, and theoretical conclusions are drawn from the investigation:

\subsection{Time Dilation is Always a Two-System Comparison}
In every case, time dilation is a comparison of the pace of time of one system relative to the pace of time of another. The relativistic nature is obscured by the current general relativity formulae. Case one demonstrates that the GTD calculatio is actually a two system comparison.

\subsection{Common GTD Formulae Assert Scale Privilege}
Current common understandings of relativity reinforce a notion of scale privilege by impartting judgement calls about scale with operations, cmparisons with abstract Quantities such as flat space-time, or neglegible factors. These are assertions of (human) scale privilege, because those same comparisons change meaning from the perspective of flatter space-time or a more neglegible value. Flat space-time is an ideal thhat is approached asymptoticly, but never reached. Negligibility is a matter of perspective, and is not absolute. 
The DTD system removes scale privilege by providing a single, unified formula for time dilation that is applicable to all systems, regardless of system geometry. This challenge is immediately alleviated by eliminating personal judgment calls, representing numbers in scientific notation form, with faithful precision, amd avoiding the mindset of negligibility. There is no mathematicaally valid way to naturally achieve division by zero error with floating point mathematics; in computer science a suffoiciently small fraction will eventually generate an out of bounds error, Because it requires too many characters to represent the number. Not because you've reached zero.

\subsection{Relative inertial density dictates time dilation betweern systems.
...
\begin{align}
\mathrm{DTD} &= \sqrt{k}, \quad \text{where } k = \frac{m_1 r_2}{m_2 r_1} \\
\mathrm{GTD} &= \frac{\sqrt{1-k_1}}{\sqrt{1-k_2}}, \quad \text{where } k_1 = \frac{m_1/r_1}{D_{\mathrm{crit}}}, \; k_2 = \frac{m_2/r_2}{D_{\mathrm{crit}}}
\end{align}

\subsection{No Deviation from General Relativity}
...
Given that no deviations in our observational effects are predicted, and that this equivalence emerges solely from general relativity, expected observations would always be consistent with general relativity.

\subsection{System Time Emerges from System Inertia}
System time is inversely proportaional to inertia about the selected axis, suggesting the time emerges from and is governed by inertia about the axis of rotation, or along the vector of motion being compared \cite{degerlia202501}. This can be validated mathematically by showing that the temporal response (delta in motion) for any system, about any axis or along any path of motion, is solely and inversely proportionally related to the inertia or moment of inertia of the system about the compared axis. Meaning that the temporal effect of torque applied or force applied is always the same delta of motion if the same moment of inertia were expressed about those dimensions. Regardless of what spatial geometry arrived at that moment of inertia about that axis.

\subsubsection{Inertia is Abstractly Defined as Time}
Inertia is already defined as the sole determinant of the change in motion resulting from the application of a force to a system. Regardless of the mass geometry of that system.

\subsubsection{Time would be Dual-mode}
Time hypothetically has two modes: rotational and linear, as do inertia and motion. Inertia is both rotational and linear.

\subsubsection{Time would be three dimensional}
Time would operate in three perpendicular spatial dimensions.

\subsubsection{Causality is an Inertial Property}
The emergence of system time from inertia, as described herein, offers an explanation for causality. In this context, causality emerges from the cumulatively conservative nature of inertia. If one subdivides an inertial system, regardless of the manner of division, as long as it's calculated along the same axis, one will always arrive at the same cumulative inertia for the system about that axis. That means the sum of the subsystems is always equal to the total of the system. That the behavior of the whole is always dictated by the behavior of the components.

\subsubsection{SR and GR would Emerge Naturally}
...
\subsection{Formulaic Similarities Further Link SR and GR}
The shared quantitative form of general relativity versus special relativity suggests a unified structure. Both GR and SR share a unified form: $\sqrt{1-k}$, where $k$ is A ratio of characteristic lengths or a "linear scale factor". In the case of special relativity, $k = v^2/c^2$ is ratio of characteristic lengths traveled per second, for example, per static unit of time. $k$ is a linear scale factor across all forms of time dilation described herein.

\subsection{DTD Accurately Predicts Asymmetrical Systems}
Effectively serves as a unified model between Schwarzschild and asymmetrical black hole systems, such as Kerr black holes. This hypothesis can be validated by calculating a system about three perpendicular axes, treating each dimension as an individual Schwarzschild solution about that axis using $D$ ($m/r$) and comparing with the Kerr field equation.

\subsection{No Material System Exists Without Spatial Distribution}
For classical and relativistic physics, every system requires spatial distribution of matter (mass). In a literal interpretation of classical physics and general relativity, a material singularity is unattainable, Because such a system would lack any spatial distribution. We know from the fallacy of the neglegible described above, one cannot simply declare perceived negligibility, round to zero in order to arrive at a division by zero. It must also be a mathematically valid operation. To exemplify further, a material singularity is also impossible under floating-point mathematics, because an infinitely precise zero is never attained except by assignment. There is no fraction so small that there is not another smaller fraction. This notion is a product of scale privilege.

\subsubsection{No Observations would Change}
These factors in and of themselves would not in any way change how systems would be observed relative to other systems. Not a single current, future, or past observation would be predicted to be different.
