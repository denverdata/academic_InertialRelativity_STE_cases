...
The conventional form presented two concerns. First, it was inaccurate in its reference points: the gravitational time dilation form $GTD = \sqrt{1-k}$ places the Schwarzschild radius at the zero point, yet the Schwarzschild radius is not the singularity. Second, it was cumbersome to work with, both in carrying the subtraction through a calculation and in the strings of values crowded close to one that it produces. Removing the ``$1-$'' resolves both concerns at once, yielding the complementary quantity $DTD = \sqrt{k}$.

That single step coincidentally exposed the Pythagorean complement already present within general relativity: $DTD$ and $GTD$ are the two orthogonal legs of a single unit relationship, $DTD^2 + GTD^2 = 1$. One can move freely back and forth between the two forms, recovering either from the other at any point, which validates the perspective and demonstrates that the two are equivalent. In this form, zero marks the singularity and $r_{s}$ takes the value $1$, a familiar reference point, and the Schwarzschild threshold (the black hole condition) is expressed as a single universal constant, $D_{crit}$, rather than a value that must be recalculated for every mass.

Together these yield a single, always real-valued scale across the full domain. The $GTD$ form is confined to the interval between zero and one and returns imaginary values for $k > 1$, whereas the $DTD$ form is guaranteed to remain real and grows without bound as $k$ increases past $r_{s}$ toward the singularity. Only the outer end, approaching flat space, is a true asymptotic approach. This keeps the working values away from the fringe of available precision, where they would otherwise crowd close to one, with practical relevance for applications such as GPS satellites and the International Space Station.
...
\begin{mdframed}
\noindent\textbf{A quick test: is it a black hole?} Because the threshold is a single constant, $D_{crit} \approx \num{6.73295e26}\,\unit{\kilogram\per\meter}$, one can estimate the black hole condition directly from the order of magnitude of $D = M/R$. Take the difference of the exponents of $M$ and $R$: if it is $27$ or more the system is guaranteed to be a black hole; if it is $25$ or less it is guaranteed not to be; and if it is exactly $26$ it may or may not be, and one should calculate further. For example, a system with $M = \num{1e30}$ and $R = \num{1e10}$ gives an exponent difference of $30 - 10 = 20$, well short of $26$, so it is not a black hole.
\end{mdframed}
